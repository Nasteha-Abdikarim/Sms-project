<?php
include "adminheader.php";
?>
<!DOCTYPE html>
<html lang="en">
<head>

    <title>employeelist</title>
    <link rel="stylesheet" href="style.css">
    <script src="https://code.iconify.design/iconify-icon/1.0.2/iconify-icon.min.js"></script>
   
</head>
<body>
<div class ="nav">
    <iconify-icon icon="zondicons:view-list" class ="iccon"></iconify-icon>

    <ul>
        <li><iconify-icon icon="bi:person-circle" class ="icon"></iconify-icon></li>
        <li><iconify-icon icon="vaadin:arrows-cross" class ="icon"></iconify-icon></li>
        <li><iconify-icon icon="material-symbols:arrow-circle-right-rounded"class ="icon"></iconify-icon></li>
    </ul>
    </div>
    <div class="main">



    <div class ="company">
     <div class ="dash1"><iconify-icon icon="material-symbols:person"></iconify-icon> Employee</div>
    <div class = "Com1">
        <h5>Home/Employee</h5>
    </div>

    </div>

    
    <div class="add">
       <button><a href="Employeeform.php">Add New</a></button>
     <div class ="search"><a href="search2.php"><input type="text" placeholder ="search"></a></div> 
    </div>

<table>


    <th>Employee ID</th>
    <th>Full Name</th>
    <th>Contact</th>
    <th>Gender</th>
    <th>Position</th>
    <th>Salary</th>
    <th>Action</th>
    <th>Action</th>

    <?php
  

      include "connection.php";

     $comQuery = " SELECT * FROM employeeform";
     $result = $dbconnection->query($comQuery);
     $table2 ="";

     if ($result){
        while($row = $result->fetch_assoc()){
            $table2 .= "<tr>";
            $table2 .= "<td>" . $row["Employeeid"] ."</td>";
            $table2 .= "<td>" . $row["Fullname"] ."</td>";
            $table2 .= "<td>" . $row["Contact"] ."</td>";
            $table2 .= "<td>" . $row["Gender"] ."</td>";
            $table2 .= "<td>" . $row["Position"] ."</td>";
            $table2 .= "<td>" . $row["Salary"] ."</td>";
            
           $deletelinked = "delete1.php?"."&Employeeid=".$row["Employeeid"]; 
           $updatelinked = "update.php?" ."&Employeeid=" .$row["Employeeid"]."&Fullname=".$row["Fullname"]
           ."&Contact".$row["Contact"]."&Gender=".$row["Gender"]."&Position=".$row["Position"]."&Salary".$row["Salary"];

           $table2.="<td>" .'<a href= "'.$updatelinked.'" class="updatebtn"><iconify-icon icon="material-symbols:edit-rounded" class="ubpdateBTN"></iconify-icon></a>'  ."</td>";
           
$table2.= "<td>".'<a href="'.$deletelinked.'" class="deleteBtn"><iconify-icon icon="ic:baseline-delete" class="deleteBtn1"></iconify-icon></a>'."</td>";


            $table2 .= "</tr>";
         
        }
        echo $table2;
       
     }

     else {
        echo $dbconnection->connect_error;
     }

    ?>
</table>




    </div>
      
  

    
</body>
</html>









